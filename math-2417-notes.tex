...
\begin{practice}{\hyperref[ans:proving-limit-with-formal-def]{1}}{Proving Limits via. $\varepsilon-\delta$ Definition}
	\label{prac:proving-limit-with-formal-def}
	Consider $f(x)=10x-6$, prove that $\displaystyle\lim_{x\to 3}f(x)=24$ using the
	$\epsilon-\delta$ definition
\end{practice}

\subsection{Evaluating Limits Analytically}\label{sec:evaluating-limits-analytically}

\subsubsection{Properties of Limits}\label{sec:limit-properties}
...
\begin{multicols}{2}[]
	...
	\begin{example}{Evaluating Basic Limits}\label{ex:evaluating-basic-limits}
		\begin{center}
			\begin{align*}
				\lim_{x\to 2}5   & =5  \\
				\lim_{x\to 4}x   & =4  \\
...
\begin{theorem}{Limit Properties}\label{th:limit-properties}
...
\begin{example}{Limit of Polynomial}\label{ex:limit-polynomial}
	\begin{align*}
...
\begin{theorem}{Limits of Polynomial/Rationals}\label{th:limit-polynomial}
...
\begin{center}
	\begin{multicols}{2}[
			There are two cases where discontinuities happen: \marginnote{Another
				example of removable discontinuity is in example~\ref{ex:estimating-limit}}
		]
...
\begin{theorem}{Derivative via. Limits}\label{th:derivative-limit-def}
	\begin{displaymath}
...
\begin{note}{Alternative Ways of Writing a Derivative}\label{note:derivative-representations}
...
\begin{theorem}{Continuity of Derivatives}\label{th:derivative-continuity}
	If function $f(x)$ is differentiable at $x = c$, then it is
	\textit{also continuous} at $ x=c $
\end{theorem}

\begin{practice}{\hyperref[ans:find-tangent-line]{1}}{Finding the Tangent Line}\label{prac:find-tangent-line}
	Find the tangent lines to $f(x) = x^2 + 1$ at $ (-2, 5) $
\end{practice}

\subsection{Rules of Derivatives}\label{sec:derivative-rules}
...
\begin{theorem}{Derivative Rules}\label{th:derivative-rules}
	\begin{align*}
...
\begin{example}{Power Rule}\label{ex:power-rule}
	\begin{multicols}{2}
		\begin{enumerate}
...
\subsection{Higher Order Derivatives}\label{sec:higher-order-derivative}
You can take a derivative of derivative:
\begin{displaymath}
	\frac{d}{dx}\big[\frac{d}{dx}f(x)\big]=\frac{d^2}{dx^2}\big[f(x)\big]
	=f''(x)
\end{displaymath}
\begin{example}{Finding the Second Derivative}\label{ex:second-derivative}
	...
	\end{align*}
\end{example}
\begin{practice}{\hyperref[ans:velocity-acceleration]{1}}{Velocity and Acceleration}
	\label{prac:velocity-acceleration}
...
\begin{note}{Taking Beyond the Third Derivative}\label{note:beyond-third-derivative}
...
\subsection{Implicit Differentiation}\label{sec:implicit-differentiation}
...
\begin{example}{Implicit Differentiation with Product Rule}\label{ex:implicit-differentiation}
...
\begin{multicols}{2}
    \begin{practice}{\hyperref[ans:trig-implicit-differentiation]{1}}{Tangent Lines and Circles}
		\label{prac:trig-implicit-differentiation}
		Find the tangent line at $(4,-3)$ of \textit{the circle}:
		$$x^2+y^2=5^2$$
	\end{practice}

	\begin{practice}{\hyperref[ans:differentiate]{2}}{Differentiate}\label{prac:differentiate}
		Find the derivative of this function: $$(4x+4y)^3=64x^3+64y^3$$
	\end{practice}
\end{multicols}

\subsection{Related Rates}\label{sec:related-rates}
...
\begin{note}{Guideline for Related-Rate Problems}\label{note:related-rates-guide}
	\begin{enumerate}
...
\begin{multicols}{2}
    ...
    \begin{note}{Finding Absolute Extrema}\label{note:find-abs-extrema}
