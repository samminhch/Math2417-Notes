...
\newpage
\marginpar{$\frac{dx}{dx}=1$. I'm only showing it in this example $\frac{dx}{dx}$}
...
\marginpar{Absolute maximum/minimum is also known as the \textit{global}
maximum/minimum}
...
\marginpar{Make sure when doing these problems that the critical values you find
...
\marginpar{Inflection points only occur when \newline$f''=0\lor DNE$. In the case of this
graph, it's at (0, 0)}
\begin{multicols}{2}
...
\marginpar{You could also apply chain rule to $(4x+4y)^3$ if you so choose}
\begin{align*}
    (4x+4y)^3&=64x^3+64y^3\\
...
\marginpar{I used these variable letters, but you can use your own if you wish!}
\begin{multicols}{2}
    \begin{itemize}
...
\marginpar{It's a 7-24-25 triangle, so $h$ is actually 24 ft!}
\begin{itemize}
...
\marginpar{Chain rule was used to find $\frac{d}{dx}(\sin^2\theta)$}
\begin{align*}
...
\marginpar{$\frac{d}{dx}\sqrt{x}=\frac{1}{2\sqrt{x}}$}
\begin{multicols}{2}
\begin{itemize}
    \item $a<c<b$
...
be applied, explain why.
\br
\marginpar{This is a little redundant, but some questions still ask for this
step}
...
\continued
\marginpar{The expression translates to: ``for all $x$ in the set of real
numbers, $f'(x)\geq3$''}
